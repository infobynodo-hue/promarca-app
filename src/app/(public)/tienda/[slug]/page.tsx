  ...
  const css = `
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .gallery-img-wrap {
      ...
      background: #fff; border-radius: 16px; overflow: hidden; aspect-ratio: 1;
      box-shadow: 0 4px 24px rgba(0,0,0,0.4);
    }
    .gallery-img-wrap img { width: 100%; height: 100%; object-fit: contain; display: block; }
    .gallery-placeholder {
      flex: 0 0 calc(100% - 40px); background: #1a1a1a; border-radius: 16px;
      aspect-ratio: 1; display: flex; align-items: center; justify-content: center;
    }
    .gallery-placeholder span { font-size: 3rem; opacity: 0.3; }
    .gallery-dots { display: flex; justify-content: center; gap: 6px; padding: 8px 0 0; }
    .gallery-dot { width: 6px; height: 6px; border-radius: 50%; background: rgba(255,255,255,0.25); }
    .gallery-dot.active { background: ${primaryColor}; width: 18px; border-radius: 3px; }

    .headline-block { padding: 24px 20px 16px; text-align: center; }
    .headline-text { font-size: 1.4rem; font-weight: 800; color: #fff; line-height: 1.3; letter-spacing: -0.02em; }
    .subheadline-text { margin-top: 8px; font-size: 0.95rem; color: #a1a1aa; line-height: 1.5; }

    .price-block { padding: 0 20px 20px; text-align: center; }
    .price-compare { font-size: 1rem; color: #71717a; text-decoration: line-through; margin-bottom: 4px; }
    .price-main { font-size: 2.4rem; font-weight: 900; color: #fff; line-height: 1; }
    .price-note { margin-top: 6px; font-size: 0.8rem; color: #52525b; }

    .cta-primary {
      margin: 0 20px 20px; display: block; width: calc(100% - 40px);
    ...
    .cta-primary.disabled { background: #3f3f46; color: #71717a; cursor: not-allowed; box-shadow: none; }

    .benefits-card {
      margin: 0 20px 20px; background: rgba(255,255,255,0.05);
    ...
    .benefit-item { display: flex; align-items: flex-start; gap: 12px; padding: 10px 0; }
    .benefit-item + .benefit-item { border-top: 1px solid rgba(255,255,255,0.07); }
    .benefit-emoji { font-size: 1.3rem; flex-shrink: 0; }
    .benefit-text { font-size: 0.9rem; color: #e4e4e7; line-height: 1.4; }

    .fomo-bar {
      margin: 0 20px 20px; background: rgba(255,107,44,0.12);
      border: 1px solid rgba(255,107,44,0.3); border-radius: 12px; padding: 12px 16px;
      display: flex; align-items: center; gap: 10px;
    }
    .fomo-dot {
      width: 8px; height: 8px; border-radius: 50%; background: ${primaryColor}; flex-shrink: 0;
      animation: pulse 1.5s ease-in-out infinite;
    }
    .fomo-text { font-size: 0.85rem; color: #e4e4e7; font-weight: 500; }

    .cta-whatsapp {
      margin: 0 20px 28px; display: block; width: calc(100% - 40px);
      height: 48px; border-radius: 12px; border: 1px solid rgba(37,211,102,0.3);
      cursor: pointer; background: rgba(37,211,102,0.08);
      color: #25D366; font-size: 0.95rem; font-weight: 700;
      text-decoration: none; text-align: center; line-height: 48px;
      box-shadow: 0 0 20px rgba(37,211,102,0.1); transition: background 0.15s;
      backdrop-filter: blur(12px);
    }
    .cta-whatsapp:hover { background: rgba(37,211,102,0.15); }

    .lp-footer { padding: 24px 20px; text-align: center; border-top: 1px solid rgba(255,255,255,0.07); }
    ...
    @media (min-width: 768px) {
      ...
      .col-left {
        ...
        padding: 40px 32px; border-right: 1px solid rgba(255,255,255,0.07);
      }
      .col-right {
        overflow-y: auto; padding: 40px 0;
      }
      .brand-header { padding: 0 32px 24px; align-items: flex-start; }
      .gallery { padding: 0; }
      .gallery-scroll { padding: 0 0 12px; }
      .gallery-img-wrap { flex: 0 0 100%; border-radius: 20px; }
      .gallery-placeholder { flex: 0 0 100%; }
      .headline-block { padding: 0 32px 20px; text-align: left; }
      .headline-text { font-size: 1.8rem; }
      .price-block { padding: 0 32px 24px; text-align: left; }
      .price-main { font-size: 2.8rem; }
      .cta-primary { margin: 0 32px 20px; width: calc(100% - 64px); }
      .benefits-card { margin: 0 32px 20px; }
      .fomo-bar { margin: 0 32px 20px; }
      .cta-whatsapp { margin: 0 32px 28px; width: calc(100% - 64px); }
      .lp-footer { padding: 24px 32px; }
    }

    /* ── DESKTOP (1024px+) ── */
    @media (min-width: 1024px) {
      .layout { grid-template-columns: 55% 45%; max-width: 1280px; }
      .col-left { padding: 60px 48px; }
      .col-right { padding: 60px 0; }
      .brand-header { padding: 0 48px 32px; }
      .headline-block { padding: 0 48px 24px; }
      .headline-text { font-size: 2.2rem; }
      .subheadline-text { font-size: 1.05rem; }
      .price-block { padding: 0 48px 28px; }
      .price-main { font-size: 3.2rem; }
      .cta-primary { margin: 0 48px 24px; width: calc(100% - 96px); font-size: 1.1rem; height: 60px; line-height: 60px; }
      .benefits-card { margin: 0 48px 24px; }
      .fomo-bar { margin: 0 48px 24px; }
      .cta-whatsapp { margin: 0 48px 32px; width: calc(100% - 96px); }
      .lp-footer { padding: 32px 48px; }
    }
